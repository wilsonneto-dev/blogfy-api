...
class UsersController {
  public async create(request: Request, response: Response): Promise<Response|undefined> {
    try {
      ...
      const createUserService = container.resolve<ICreateUserService>('CreateUserService');
      ...
      if(error instanceof EmailAlreadyExistsException)
        throw new AppHttpError((error as EmailAlreadyExistsException).message, HttpStatusCode.Conflict);
    }
  }
}
